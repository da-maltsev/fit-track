<script lang="ts">
    ...
    import type { ExerciseList, TrainingExerciseCreate, TrainingRead } from '$lib/types';
    import { onMount } from 'svelte';
    import { page } from '$app/stores';

    let exercises: ExerciseList[] = [];
    let training: TrainingRead | null = null;
    let loading = true;
    let error = '';
    let date = '';
    ...
    async function loadData() {
        try {
            loading = true;
            error = '';
            const [exercisesData, trainingData] = await Promise.all([
                api.listExercises(),
                api.getTraining(Number($page.params.id))
            ]);
            exercises = exercisesData;
            training = trainingData;
            date = new Date(training.date).toISOString().split('T')[0];
            selectedExercises = training.exercises.map((e) => ({
    ...
    async function handleSubmit() {
        ...
        try {
            loading = true;
            error = '';
            await api.updateTraining(Number($page.params.id), {
